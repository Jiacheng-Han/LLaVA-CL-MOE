...
class MOELoraLinear(nn.Module, MOELoraLayer): 
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs):
        ...
        # 4. Softmax
        router_weights = F.softmax(router_logits / self.router_temperature, dim=-1).to(x_lora.dtype)
        ...
        if self.training:
            ...
            for i in range(self.current_expert_num):
                ...
                if i != target_expert_idx:
                    # 【硬隔离底线】：旧专家绝对不更新，物理切断反向传播！
                    expert_out = expert_out.detach()
                    # 【软结合惩罚】：用户的神来之笔！强行把非当前专家的干扰降低一半
                    weight_i = weight_i * 0.5
            ...
            # -------------------------------------------------------------
            # 【上帝视角测试 Hack】：如果要强行测任务一，请解开下面两行的注释！
            # 注意：如果解开注释，它会覆盖上面辛辛苦苦算出来的安全 router_weights
            # router_weights = torch.zeros_like(router_logits)
            # router_weights[..., 0] = 1.0  # (测任务一写0，测任务二写1)
            # -------------------------------------------------------------
