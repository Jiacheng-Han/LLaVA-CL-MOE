...
class LLaVATrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        # 1. 强制先调用 HF Trainer 原生的保存逻辑
        # 这会自动帮你保存 LoRA 权重 (adapter_model.bin)、优化器状态 (optimizer.pt) 等，支持断点续训
        super(LLaVATrainer, self)._save_checkpoint(model, trial, metrics)

        # 2. 额外把 Projector 的权重也塞进刚才生成的 checkpoint 文件夹里
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            if self.args.local_rank == 0 or self.args.local_rank == -1:
                # 只保存 projector.bin，不去覆盖 config
                torch.save(weight_to_save, os.path.join(output_dir, f'mm_projector.bin'))

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        # 同理，解除原有的 pass 拦截，强制调用父类保存
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # 2. 收集模型中所有 MOE LoRA 层的 Router Logits，计算 Routing Loss
        if getattr(self.args, 'lora_enable', False):
            ...
            if layer_count > 0:
                ...
                # 3. 将 Routing Loss 加到总 Loss 中
                loss = loss + alpha * routing_loss
